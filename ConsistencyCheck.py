...
if not os.path.exists(TName + '_phedex.json') or opts.newPhedexOnly:
    ...
    print preFix + stripFile(repl['name']) == lastDirectory
    blockList.append({'dataset':block['name'],'directory':preFix + stripFile(repl['name']),'files':tempBlock})
    del inData
