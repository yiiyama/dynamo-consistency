...
#    origtime=`/home/dabercro/./jq -M .phedex.request_timestamp $site/$site\_added.json | sed 's/\..*$//'`
    ...
    if [ $oldtime -gt $origtime ]; then
        requesttime=`expr $origtime - 605000`       # Request data one week older than the previous request
        requestdelete=`expr $origtime - 1210000`    # Request deletions two weeks older than the previous request
