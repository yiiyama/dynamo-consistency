...
declare -a sites=("T2_EE_Estonia" "T2_ES_CIEMAT" "T2_IT_Pisa" "T2_IT_Rome" "T2_US_Caltech" "T2_US_Florida" "T2_US_Nebraska" "T2_US_Purdue" "T2_US_UCSD" "T2_US_Vanderbilt" "T2_US_Wisconsin")
...
#    origtime=`/home/dabercro/./jq -M .phedex.request_timestamp $site/$site\_added.json | sed 's/\..*$//'`
    ...
    if [ $oldtime -gt $origtime ]; then
        ...
        /home/dabercro/./jq -M -s '[.[]|.phedex|.block[]|{directory:.file[0].name|split("/")[0:-1]|join("/"),files:[.file[]|{time:.time_create,adler32:.checksum|split(",")[0]|split(":")[1],file:.name|split("/")[-1:][0],size:.bytes}],dataset:.name}]' $site/PhEDEx/*.json > $site/$site\_prephedex.json
